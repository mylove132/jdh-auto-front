...
export default function useModal(props, { emit, attrs }: SetupContext) {
  ...
  function resize(
    ...
    handle.onmousedown = function (event) {
      ...
      const mousemove = event => {
        ...
        lockY ||
          (modalBody.value.style.height =
            iH - footerHeight - headerHeight + 'px');
  ...
  const initWin = () => {
    ...
    dragRef.value.style.left =
      (document.documentElement.clientWidth - dragRef.value.offsetWidth) / 2 +
      'px';
    dragRef.value.style.top = props.centered
      ? (document.documentElement.clientHeight - dragRef.value.offsetHeight) /
          2 +
        'px'
      : '100px';
